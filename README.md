...
- [Für neue Entwickler](#-für-neue-entwickler)
...
│   ├── dateUtils.ts              # Zentrale Datums-Funktionen (Timezone-Handling)
│   ├── messages.ts               # Zentrale Fehlermeldungen (Deutsch)
│   ├── permissions.ts            # Berechtigungsdefinitionen & Default-Rollen
│   ├── checkPermission.ts        # Berechtigungsprüfung-Helpers
│   └── statistics.ts             # Statistik-Berechnungen
│
├── types/                        # TypeScript-Typdefinitionen
│   ├── index.ts                  # Zentrale Exports
│   ├── downtime.ts               # Downtime, DowntimeStatus
│   ├── master-data.ts            # SystemCategory, Anlageteil, Grund
│   ├── user.ts                   # User, Role
│   └── mail.ts                   # MailRecipient, MailTemplate
│
...
Dieser Abschnitt hilft neuen Entwicklern, sich schnell im Projekt zurechtzufinden.

### Wichtige Dateien zum Einstieg

| Datei | Beschreibung |
|-------|--------------|
| `src/types/index.ts` | Zentrale TypeScript-Typen - hier starten! |
| `src/lib/dateUtils.ts` | Datum/Zeit-Funktionen mit "Fake UTC" für Timezone-Konsistenz |
| `src/lib/messages.ts` | Alle deutschen Fehlermeldungen zentral verwaltet |
| `src/lib/permissions.ts` | Berechtigungssystem mit 40+ granularen Rechten |
| `src/components/Sidebar.tsx` | Navigation - hier neue Menüpunkte hinzufügen |
| `prisma/schema.prisma` | Datenbank-Schema mit allen Models |

### Code-Konventionen

```typescript
// ✅ Typen aus @/types importieren
import { Downtime, User, Anlageteil } from '@/types'

// ✅ Fehlermeldungen aus @/lib/messages verwenden
import { MESSAGES } from '@/lib/messages'
setError(MESSAGES.errors.saveFailed)

// ✅ Datumsfunktionen aus @/lib/dateUtils verwenden
import { getFakeUtcNow, createFakeUTCDate } from '@/lib/dateUtils'
```

### Timezone-Handling (Wichtig!)

Das System verwendet "Fake UTC" für konsistente Zeitdarstellung:
- Lokale Zeit wird mit `Z`-Suffix gespeichert
- Alle Datumsfunktionen sind in `src/lib/dateUtils.ts` zentralisiert
- Bei `toLocaleTimeString()` immer `timeZone: 'UTC'` angeben

### Neue Seite hinzufügen

1. Seite erstellen: `src/app/admin/[name]/page.tsx`
2. Navigation: `src/components/Sidebar.tsx` → neuen Menüeintrag hinzufügen
3. Berechtigung (optional): `src/lib/permissions.ts` → neue Permission definieren
4. API (optional): `src/app/api/[name]/route.ts`
...
npx prisma migrate   # Schema-Änderungen anwenden
...
- `src/app/api/` → Backend API-Routen
- `src/components/` → Wiederverwendbare React-Komponenten
- `src/lib/` → Hilfsfunktionen & Utilities
...
Dieses Projekt wurde von E. Ugurcan Cam unter Einsatz von KI-Technologien und eigenständigem Fachwissen entwickelt.
